...
function require(mod)
	...
	local path = ('%s/%s.lua'):format(context, mod)
	local file = LoadResourceFile(GetCurrentResourceName(), path)
	if file then
		local method, err = load(file, ('@@%s/%s.lua'):format(GetCurrentResourceName(), path))
		if not method then
			return error(err)
		end

		modules[mod] = method() or true
